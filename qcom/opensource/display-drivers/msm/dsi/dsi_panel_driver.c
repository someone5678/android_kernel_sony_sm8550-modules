...
static ssize_t dsi_panel_driver_maxbrightness_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct dsi_display *display = dev_get_drvdata(dev);
	return scnprintf(buf, PAGE_SIZE, "%u", display->panel->spec_pdata->maxbrightness);
}
...
static struct device_attribute panel_attributes[] = {
	...
	__ATTR(somc_max_brightness, S_IRUSR, dsi_panel_driver_maxbrightness_show, NULL),
};
